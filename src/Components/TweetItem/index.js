...
const TweetItem = ({ name, tag, content, time, admin, reply,liked }) => {
  return (
    ...
    {reply && <div className={styles.reply}>回覆<span>@ apple</span></div>}
    <div className={styles.content}>{content}</div>
    {!admin&&!reply && <div className={styles.interact}>< Talk className={styles.icon}/><span>13</span>{!liked?<Like className={styles.icon} />:<Liked fill="#FC5A5A;" className={styles.liked} />}<span>13</span></div>}
    </div>
